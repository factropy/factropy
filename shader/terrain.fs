...
//float fogOffset = 950.0f;
//vec4 fogColor = vec4(0.4,0.74,1.0,1.0);
...
void main() {
    ...
    vec4 texelColor = groundColor;

    if (fragPosition.y > 0.0) {
        vec4 terrainColor = vec4(0.45,0.4,0.4,1.0);
        if (fragPosition.y > 3.0) {
            texelColor = terrainColor;
        } else {
            texelColor.x = mix(texelColor.x, terrainColor.x, (fragPosition.y/3.0));
            texelColor.y = mix(texelColor.y, terrainColor.y, (fragPosition.y/3.0));
            texelColor.z = mix(texelColor.z, terrainColor.z, (fragPosition.y/3.0));
        }
    }

    if (fragPosition.y < 0.0) {
        vec4 terrainColor = vec4(0.75,0.5,0.25,1.0);
        if (fragPosition.y < -3.0) {
            texelColor = terrainColor;
        } else {
            texelColor.x = mix(texelColor.x, terrainColor.x, -(fragPosition.y/3.0));
