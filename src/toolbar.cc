...
void Toolbar::draw() {
	...
	Begin("##toolbar", nullptr, flags);
		...
		if (scene.directing) {
			...
			if (ImageButton(Popup::specIconChoose(scene.directing->spec, iconSize), ImVec2(iconPix, iconPix), ImVec2(0, 1), ImVec2(1, 0))) {
		...
		for (auto spec: buttons) {
			...
			if (i) SameLine();
			if (ImageButton(Popup::specIconChoose(spec, iconSize), ImVec2(iconPix, iconPix), ImVec2(0, 1), ImVec2(1, 0))) {
				scene.build(spec);
			}
			if (IsItemHovered()) {
				tip = spec->title;
